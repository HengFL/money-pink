...
export const CentralMoneyDashboard = ({ data, availableYears, selectedYear, onYearChange, availableMembers, selectedMember, onMemberChange, onRefresh }) => {
  ...
  const centralMoneyMetrics = [
    { key: 'called', label: 'ยอดเรียก', color: '#db2777' },
    { key: 'collected', label: 'ยอดเก็บ', color: '#15803d' },
    { key: 'outstanding', label: 'ยอดค้าง', color: '#dc2626' },
    { key: 'borrowed', label: 'ยอดยืมเงิน', color: '#1d4ed8' },
    { key: 'returned', label: 'ยอดคืนเงิน', color: '#0e7490' },
    { key: 'outstandingReturn', label: 'ยอดค้างคืน', color: '#ea580c' }
  ];
      ...
      <div style={{ marginTop: 'var(--spacing-xl)' }}>
        ...
        {memberViewMode === 'list' ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-md animate-fade-in">
            {centralMoneyMetrics.map(metric => (
              <div key={metric.key} className="bg-card" style={{ padding: '1rem', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', boxShadow: 'var(--shadow-sm)' }}>
                ...
                <div style={{ display: 'flex', width: '100%', flex: 1, alignItems: 'center' }}>
                  <div style={{ width: '50%', minWidth: 0, display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '0.5rem 1rem 0.5rem 0.5rem', borderRight: '1px dashed var(--border-color)' }}>
                    <ReactApexChart 
                      options={{
                        ...
                        labels: members.filter(m => m.name !== 'ตัวแทน').map(m => m.name),
                      ...
                      series={members.filter(m => m.name !== 'ตัวแทน').map(m => m.totals[metric.key] || 0)}
                      type="donut"
                      height={180}
                      width="100%"
                    />
                  ...
                  <div style={{ width: '50%', maxHeight: '180px', overflowY: 'auto', paddingLeft: '1rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                    {members.filter(m => m.name !== 'ตัวแทน').map((m, i) => {
